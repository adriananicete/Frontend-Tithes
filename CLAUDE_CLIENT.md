...
- **2026-04-26** — **🚀 PRODUCTION DEPLOYMENT LIVE.** App is fully deployed end-to-end. Login at `https://tithes-management-system.vercel.app` works against real backend + real DB. **Stack:** Frontend on Vercel, Backend on Render (free tier), DB on MongoDB Atlas (free M0 cluster). **Live URLs:** Frontend `https://tithes-management-system.vercel.app` · Backend `https://backend-tithes-management-system.onrender.com` (root `/` returns "Hello World!" for smoke testing) · API base `https://backend-tithes-management-system.onrender.com/api`. **Atlas:** cluster `cluster0.ckd3rkk.mongodb.net`, database name `JOSCM-Tithes`, db user `aniceteian14_db_user`. Network access set to `0.0.0.0/0` (Render free tier has no static outbound IPs; security relies on user/password + TLS via `mongodb+srv://`). **Vercel env vars set:** `VITE_API_URL=https://backend-tithes-management-system.onrender.com/api` (Production + Preview + Development). Note: must include `/api` suffix — `apiFetch(path)` does `${BASE_URL}${path}` and route paths like `/auth/login` are mounted under `/api/*` on backend. Initial deploy attempt without `/api` 404'd at `/auth/login`. **Render env vars set:** `CONNECTION_STRING` (Atlas + `/JOSCM-Tithes` database), `JWT_SECRET_KEY=my-secret-key`, `CLOUDINARY_NAME/API_KEY/API_SECRET`, `CORS_ORIGIN=https://tithes-management-system.vercel.app`. `PORT` deliberately omitted — Render auto-injects its own and `app.js` reads `process.env.PORT`. **Render service config:** Build Command `npm install`, Start Command `node app.js` (manual override since `package.json` has no `start` script — see pending follow-up #3 below), Instance Type Free, Region Singapore, Branch `main`. **Admin seeded directly into Atlas:** ran `node --env-file=.env src/utils/seed.js` once with backend `.env` temporarily pointed at the Atlas connection string + `seed.js` password temporarily changed from `admin123` → `adrian`; both files reverted after seed succeeded so the working tree stayed clean. **Production credentials (weak — change soon):** `adrian@joscm.com` / `adrian`. **Cold start behavior:** Render free tier sleeps after ~15 min idle; first request after a cold period takes 30–60s before the Node process responds. Any subsequent request (within ~15 min) is fast. **PENDING FOLLOW-UPS** *(intentionally deferred — Adrian wants to ship the working deploy first, polish after):*
  ...
  **PENDING FEATURE WORK** *(separate from deployment polish above — these are unbuilt features, not config tweaks):*
  - **A. Reports page real-data wiring** — `src/pages/Reports.jsx` still imports `mockTithesData` / `mockExpenseData` from `reports-components/mockData` (verified at `Reports.jsx:10-13` and `:36`); this is the **last unwired feature page** in the app. Backend ready: `GET /api/reports/tithes` (all roles, member sees own only), `GET /api/reports/expense` (admin/auditor), and the four `/export/excel` + `/export/pdf` endpoints per `CLAUDE.md`. Follow the codified Real Data Rollout pattern in `memory/project_real_data_rollout.md`: new `src/hooks/useReports.js` owning the fetch + date-range + tab params; `Reports.jsx` mounts the hook + filters tab to admin/auditor for expense; display components (`ReportPreviewTable`, `ReportSummary`, `ExportBar`) accept arrays as props and recompute via `useMemo`. Export buttons hit the export endpoints directly with `Authorization` header (existing `apiFetch` pattern works since the backend returns binary streams; may need to either extend `apiFetch` to handle blob responses, or do a one-off `fetch()` in the export handler). Should be a single focused branch — `feat/reports-real-data` — fits the established 1-feature-per-branch shape.
  - **B. Notifications page + bell icon — net-new feature, not just wiring** — currently nothing in the frontend renders notifications: no `useNotifications.js` hook exists; no `Notifications.jsx` page exists; `Header.jsx` has no bell icon (verified — only stale icon imports for unrelated UI). Backend is fully built: `GET /api/notifications` (auto-filtered to JWT user), `PATCH /api/notifications/:id/read`, `PATCH /api/notifications/read-all`, plus the auto-trigger system that fires a notification on every status change (tithes approved/rejected, RF validated/approved/rejected/voucher_created/disbursed, voucher created — see `CLAUDE.md` § Notification System for the full event matrix). Build scope: (1) `src/hooks/useNotifications.js` — `GET` + mark-read + mark-all-read mutations + unread count derived via `useMemo`; (2) `src/pages/Notifications.jsx` — list view with filter (all / unread), each row links to its `refModel`+`refId` (Tithes / RequestForm / Voucher) so clicking jumps to the relevant page; (3) bell icon in `Header.jsx` with unread count badge and a dropdown showing the 5 most recent unread, "Mark all as read" button, "View all" link to `/notifications`; (4) new route `/notifications` in `App.jsx` (all authenticated roles). Polling vs websockets: **start with polling** (e.g., 30-60s `setInterval` in the hook). Socket.IO can come later if real-time matters; for an internal church app with ~7 users, polling is fine and simpler. The polling cadence is also what the scheduled RF inline-comments agent will inherit, so picking it now sets the pattern.
  - **C. Why these two are linked → RF inline comments planning** — both A and B are **prereqs** for the scheduled remote agent firing **2026-05-14T01:00:00Z (09:00 PHT)** that will evaluate building realtime chat threads on Request Forms (so requestor + approvers can clarify questions during the approval flow). The agent will check if `feat/reports-real-data` is merged + Notifications page exists with bell + `useNotifications` hook exists; if all three pass, it drafts a plan that **reuses whatever realtime channel notifications shipped with** (polling or Socket.IO) instead of introducing a second mechanism. If any prereq is missing on May 14, the agent posts a "skipped, prereqs not done" issue and exits. So shipping A + B before May 14 unlocks an automated planning pass. Routine: `trig_01Gywchh72gTtrWAyAohWR6u` (see `memory/project_rf_comments_scheduled.md` for full context).

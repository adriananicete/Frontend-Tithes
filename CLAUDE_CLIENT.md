...
- **IN PROGRESS (started 2026-04-20)** — `feat/voucher-real-data` (off `main`). Swapping Voucher page + Dashboard Create Voucher button to real `/api/vouchers` with multipart receipt uploads. Backend Cloudinary flow already shipped in `Backend-Tithes#feat/voucher-cloudinary-receipts` (merged 2026-04-20). Full 11-step plan + 5-commit strategy in memory `project_voucher_real_data_plan.md` — if this session gets cut, continue from the next unchecked step by reading that memory plus `git log origin/feat/voucher-real-data..HEAD`. This bullet will be replaced with a normal merged entry when the branch lands.

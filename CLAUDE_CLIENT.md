...
- **2026-04-19** — `feat/dashboard-role-quick-actions` pushed. Replaces the single hardcoded "Add Category" button on the Dashboard header with a role-aware quick-actions row that navigates to the relevant feature page. (1) **`src/pages/Dashboard.jsx`** — defines `ADD_CATEGORY` / `CREATE_VOUCHER` / `SUBMIT_TITHES` / `GENERATE_REPORT` constants (label + react-icon + target path) and a `QUICK_ACTIONS_BY_ROLE` map so each role only sees shortcuts they can actually execute: admin → all 4; validator → Create Voucher + Submit Tithes; DO + member → Submit Tithes only; auditor → Generate Report; pastor → none (the button block is omitted entirely). Uses `useAuth().user.role` and `useNavigate()` from `react-router`; the outer header's `justify-between` still works when actions are empty (pastor) because the title column is the lone flex child. (2) **Button consistency** — each action wraps the existing `@/components/Buttons` black-CTA component in a `w-full sm:w-40` clickable div, matching the original header pattern; row stacks vertically on mobile via `flex flex-col sm:flex-row`. (3) **Policy** — mapping intentionally mirrors the backend ACL (`utils/rolePermissions.js#can`) rather than a wish-list. Notably, DO (Jaymar) does **not** get Create Voucher on the dashboard — backend `can.createVoucher` is validator+admin only, so putting the shortcut on DO's dashboard would produce a 403 on submit. (4) **No new dependencies**, no route changes — all four target paths already exist and are already role-protected via `ProtectedRoute`.

...
- **2026-04-27** — `fix/dialog-input-bugs` pushed. Two unrelated dialog input bugs Adrian hit while testing notifications, root-caused and fixed in one branch. (1) **`ChangePasswordDialog.jsx`** — every keystroke registered the first character then dropped focus; subsequent keys went nowhere because no field was focused. Root cause: `PasswordField` was defined **inside** the `ChangePasswordDialog` component body. React saw a new function reference every parent render, so it unmounted and remounted the inputs on every keystroke (with focus loss as the side effect). The classic "inline component" anti-pattern — a function-as-component declared inside another component's body has unstable identity. Fix: hoisted `PasswordField` to module scope above the dialog component. Three call sites unchanged. The fix has zero behavioral risk because `PasswordField` was already pure (only reads its props, no closure over parent state) — it was just wrongly scoped. **Why this slipped past `feat/form-validation-hints` review:** the dialog worked manually during that branch's testing because the bug only manifests when a parent re-render happens between keystrokes; during that branch most testing was for `<FieldHelp>` rendering, not character-by-character typing under live error-state updates. (2) **`SubmitTithesDialog.jsx`** — denomination quantity inputs displayed a literal `0` as a value (not a placeholder), so users had to manually clear before typing. Root cause: initial `qtys` state seeded each denomination with the **number** `0`. Line 161 bound `value={qty}` directly. With a numeric `0` as the value, the placeholder attribute was suppressed (HTML5 `<input>` shows placeholder only when value is empty). Fix: initial seed and `reset()` now use `""` instead of `0`. `setQty` got a special case — when `value === ""` (user cleared the field), state preserves `""` instead of coercing to `0`; otherwise the existing `parseInt(value, 10) || 0` pipeline runs. Render uses `value={qty}` directly (now empty when not yet typed) and derives `numericQty = Number(qty) || 0` for the subtotal math (handles both string and number cases gracefully). Added `placeholder="0"` for the visual hint. **Wire shape unchanged:** `total` derivation still does `qtys[b] || 0` (works with empty strings via JS coercion). Submit payload still filters `(qtys[bill] || 0) > 0` and the kept entries are always numbers. Backend contract is identical. **`vite build` clean** (6.79s). **Net change:** 2 files, 38 insertions / 32 deletions. **Trigger:** Adrian noticed both bugs in real testing. Both fixes are minimum-viable — no broader refactor.

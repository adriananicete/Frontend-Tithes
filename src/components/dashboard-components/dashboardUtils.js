  ...
  for (const t of tithes) {
    ...
    if (t.status === "pending") {
      ...
      items.push({
        ...
        userId: t.reviewedBy?._id ?? t.submittedBy?._id,
  ...
  for (const rf of rfs) {
    ...
    if (rf.status === "submitted") {
      ...
      items.push({
        ...
        userId: rf.receivedBy?._id ?? rf.requestedBy?._id,
